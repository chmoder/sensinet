...
main(int Parm_Count, char *Parms[])
{
   ...
   {                                    //run the program
      ...
      //File for recieved data
      FILE *data;
      //Array for one liners
      char one_line[64];
      //Counter
      int counter = 0;
               ...
               for (i=0; i<res; i++)  //for all chars in string
               {
                  ...
                  switch (Format)
                  {
                     ...
                     case 5:         //asc
                        ...
                        if(isdigit((int)In1) || (char)In1 == '.')
                        {
                            one_line[counter++] = (int)In1;
                        }
                        else if((char)In1 == '\r')
                        {
                           //data = fopen("data","a");
...
void updateDb(float count) 
{
    time_t seconds;
    seconds = time(NULL);

    char *query = "INSERT INTO sensinet(time, pulse) VALUES(?,?)";
    MYSQL *conn;
    MYSQL_RES *res;
    MYSQL_ROW row;
    MYSQL_BIND bind[2];
    ...
    /* send SQL query */
    /*
       if (mysql_query(conn, "INSERT INTO sensinet (pulse) VALUES()")) 
       {
       fprintf(stderr, "%s\n", mysql_error(conn));
       exit(1);
       }
